...
it('renders default group sections expanded with exactly one card per extension',()=>{
 const noop=()=>{};
 const html=renderToStaticMarkup(createElement(ExtensionCards,{activeGroup:'all',filtered:false,rows:seedState.extensions,groups:seedState.groups,selected:[],menu:null,onSelect:noop,onDropExtensions:noop,onMenu:noop,onToggle:noop,onUpdate:noop,onEditTags:noop,onFilterTag:noop,onMove:noop,onDetail:noop,onCopy:noop}));
