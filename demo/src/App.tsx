...
export default function App(){
 ...
 return <div className="app" onKeyDown={e=>{if(e.key==='Escape'){setMenu(null);for(const ref of [tagMenu,groupMenu])if(ref.current?.open){ref.current.removeAttribute('open');ref.current.querySelector('summary')?.focus()}}}}><main>
  ...
  <DiscoveryStatus state={state} ready={ready} empty={rows.length===0} hasGroupSections={hasGroupSections} defaultVisible={status==='Visible'&&group==='all'&&!query.trim()&&!tagFilter.length} native={isNative} onClear={clear} onHistory={()=>{clear();setStatus('NotVisible')}} onRefresh={refresh}/>
