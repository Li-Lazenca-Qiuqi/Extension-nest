...
export default function App(){
 ...
 const groupMeasure=useRef<HTMLSpanElement>(null);
 const tagMeasure=useRef<HTMLSpanElement>(null);
 const [filterWidths,setFilterWidths]=useState({group:120,tags:100});
 ...
 // 分别测量两类选项，保留勾选、箭头、内边距及 16px 滚动条空间。
 useLayoutEffect(()=>{const groupElement=groupMeasure.current;const tagElement=tagMeasure.current;if(!groupElement||!tagElement)return;const width=(element:HTMLElement)=>Math.min(220,Math.max(80,Math.ceil(element.getBoundingClientRect().width)+66));const measure=()=>{const group=width(groupElement),tags=width(tagElement);setFilterWidths(current=>current.group===group&&current.tags===tags?current:{group,tags})};measure();if(typeof ResizeObserver==='undefined')return;const observer=new ResizeObserver(measure);observer.observe(groupElement);observer.observe(tagElement);return()=>observer.disconnect()},[state.groups,tags.join('\n'),tagFilter.join('\n')]);
 ...
 return <div className="app" onKeyDown={e=>{if(e.key==='Escape'){setMenu(null);for(const ref of [tagMenu,groupMenu])if(ref.current?.open){ref.current.removeAttribute('open');ref.current.querySelector('summary')?.focus()}}}}><main>
  ...
  {rows.length===0&&<div className="discovery-empty" role="status">{!ready||state.freshness==='Loading'?'Discovering extensions…':state.extensions.length>0?'No matching extensions. Clear filters to see all records.':state.freshness==='Error'?'Discovery failed. Refresh to try again.':state.freshness==='Stale'?'Discovery is stale. Refresh to try again.':'No extensions are visible to this local host yet.'}</div>}
  ...
  <ExtensionCards onSortAction={dispatch} collapsedGroups={collapsedGroups} onToggleGroup={id=>setCollapsedGroups(current=>({...current,[id]:!current[id]}))} twoColumns={twoColumns} icons={icons} isNative={isNative} activeGroup={group} filtered={Boolean(query.trim()||status!=='all'||tagFilter.length)} rows={rows} groups={state.groups} selected={selected} onSelect={setSelected} onDropExtensions={(ids,groupId,beforeId)=>{dispatch({type:'move',ids,groupId,beforeId});setSelected([])}} menu={menu} onMenu={setMenu} onOpenExtension={openExtension} onCopy={copy} onEditTags={id=>open({kind:'tags',id})} onFilterTag={toggleTag}/>
  </section></main>
