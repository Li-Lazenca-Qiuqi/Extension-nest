...
export default function App(){
 ...
 return <div className="app" onKeyDown={e=>{if(e.key==='Escape')setMenu(null)}}><main>
  ...
  <section aria-label="Installed extensions"><div className="list-toolbar"><h2>{state.groups.find(g=>g.id===group)?.name??(group==='ungrouped'?'Ungrouped':'All extensions')}<small>{rows.length}</small></h2><div className="filters">
  ...
  <ExtensionCards rows={rows} groups={state.groups} selected={selected} onSelect={setSelected} menu={menu} onMenu={setMenu} onToggle={id=>{dispatch({type:'toggle',id});setMenu(null)}} onUpdate={id=>{dispatch({type:'update',id});setMenu(null)}} onMove={id=>open({kind:'move',id})} onDetail={id=>open({kind:'details',id})} onCopy={copy} onEditTags={id=>open({kind:'tags',id})} onFilterTag={toggleTag}/>
  </section></main>
