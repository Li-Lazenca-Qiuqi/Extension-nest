...
export default function App(){
 ...
 const metrics=[['All',state.extensions.length,'all'],['Visible',state.extensions.filter(e=>e.visibility==='Visible').length,'Visible'],['Not found',state.extensions.filter(e=>e.visibility!=='Visible').length,'NotVisible'],['Ungrouped',state.extensions.filter(e=>!e.groupId).length,'ungrouped']] as const;
 ...
 return <div className="app" onKeyDown={e=>{if(e.key==='Escape'){setMenu(null);tagMenu.current?.removeAttribute('open')}}}><main>
  ...
  </div><div className="header-actions">{isNative&&<button className="cleanup-unverified" disabled={!ready||state.readOnly||state.freshness!=='Ready'||unverifiedCount===0} title="Preview old records eligible for permanent cleanup. No backup is created." onClick={cleanupUnverified}>Clear old records ({unverifiedCount})</button>}<button aria-label="Expand all groups" title="Expand all groups" onClick={()=>setCollapsedGroups({})}><GroupExpansionIcon expand/></button><button aria-label="Collapse all groups" title="Collapse all groups" onClick={()=>setCollapsedGroups(Object.fromEntries([...state.groups.map(g=>g.id),'ungrouped'].map(id=>[id,true])))}><GroupExpansionIcon/></button><button aria-label="Two-column groups" title="Two-column groups" aria-pressed={twoColumns} className={twoColumns?'view-toggle active':'view-toggle'} onClick={()=>setTwoColumns(value=>!value)}><Columns2 size={15}/></button><button onClick={()=>open({kind:'create'})}><Plus size={15}/>Group</button></div></header>
