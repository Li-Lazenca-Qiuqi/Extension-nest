...
export default function App(){
 ...
 return <div className="app" onKeyDown={e=>{if(e.key==='Escape'){setMenu(null);for(const ref of [tagMenu,groupMenu])if(ref.current?.open){ref.current.removeAttribute('open');ref.current.querySelector('summary')?.focus()}}}}><main>
  ...
  {(query||group!=='all'||status!=='all'||selected.length>0||tagFilter.length>0)&&<div className="selection-bar"><span>{selected.length?t('filter.selected',{count:selected.length}):t('filter.filters')}</span>{tagFilter.map(tag=><button key={tag} className="tag-chip" aria-label={t('filter.clearTag',{tag})} onClick={()=>toggleTag(tag)}>{tag} ×</button>)}{selected.length>0&&<><button onClick={()=>open({kind:'tags',ids:[...selected]})}><Tag size={14}/>{t('filter.editTags')}</button></>}<button onClick={clear}>{t('filter.clear')}</button></div>}
  {rows.length===0&&(!hasGroupSections||!ready||state.freshness==='Loading'||state.freshness==='Error'||state.freshness==='Stale')&&<div className="discovery-empty" role="status">{!ready||state.freshness==='Loading'?t('empty.discovering'):state.extensions.length>0?t('empty.noMatch'):state.freshness==='Error'?t('empty.discoveryFailed'):state.freshness==='Stale'?t('empty.discoveryStale'):t('empty.noVisible')}</div>}
