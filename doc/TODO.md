...
- [ ] 在真实 VS Code 中人工确认拖动手势：覆盖组标题、组内插件条目、Ungrouped 成员和多选移动，并检查悬停视觉反馈；成员目标处理已实现且控制器测试通过，整组高亮受原生 API 限制。
...
- [ ] 验证更新查询、版本和自动更新后的同步，不以网络失败推断无更新；正式接入后打开 Dashboard 自动查询一次，不设置 Check updates 按钮。
