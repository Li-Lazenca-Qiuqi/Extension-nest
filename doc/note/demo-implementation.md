...
DemoState 是包含示例扩展、唯一分组和 `tags: string[]` 的演示快照，不等于正式 [PRD-009](../archive/2026-09-17-prd/009-persistence.md) 的本地配置。旧 Demo 快照缺少 `tags` 时，正式迁移要求自动补为 `[]`，同时保留原 `groupId`；该迁移尚未验证。宿主使用独立 globalState 键保存并同步两种界面，启停和更新动作只作用于示例数据；本插件不提供卸载入口，对 VS Code 外部安装/卸载只做只读同步说明。Webview 消息经宿主校验，不能直接修改真实插件。
