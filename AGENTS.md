...
3. [PRD-000](doc/PRD/000-overview.md)：需求索引；PRD-001～PRD-009、PRD-011、PRD-015～PRD-017 为当前模块需求；PRD-010、PRD-012、PRD-013 与 PRD-014 仅保留撤销占位和历史链接。
...
- 当前范围与验收以决策 004 和活动 PRD 为准；历史完整安装/管理门槛已撤销，发现成功不等于安装全量认证。
...
- 正式本地配置以稳定 ID、唯一 assignments 和每个扩展的 tags 表达归属与标签；格式以 PRD-009 为准；真实发现历史单独缓存，不能从 Demo/seed 伪造，不与 Demo 全快照混用。
